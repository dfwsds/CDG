...
class NeuralLTO(nn.Module):
    def __init__(self, dim, meta_input_dim, n_mani=5, hidden=32):
        super().__init__()
        self.dim = dim
        self.n_mani = n_mani
...
def main(args, device):
    ...
    # Training
    lw1, lw2, lw3, lw4, lw5 = 1000, 1000, 100, 10, 100
    num_epochs = 50
    ...
    for epoch in range(num_epochs):
        epoch_loss = 0.0
        shared_model.train()
        for batch_idx in np.array_split(train_idx, len(train_idx) // 10):
            optimizer.zero_grad()
            ...
            total_loss = lw1 * loss_1 + lw2 * loss_2 + lw3 * loss_recon + lw4 * loss_embed + lw5 * loss_consis + 0.01*dscp_norm
            total_loss.backward()
            optimizer.step()
...
if __name__ == "__main__":
    ...
    args.data_path = './data/Arxiv/dataset.pkl'
    args.save_dir = './save/' + 'pred_arxiv_' + datetime.now().strftime("%y%m%d%H%M%S%f")[:-3]
    device = f"cuda:{args.device}" if torch.cuda.is_available() else "cpu"
    os.makedirs(args.save_dir, exist_ok=True)
    currentPython = sys.argv[0]
    shutil.copy2(currentPython, args.save_dir)
    shutil.copy2('utils_arxiv.py', args.save_dir)
