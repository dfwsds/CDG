...
def main(args, device):
    ...
    # Training
    lw1, lw2, lw3, lw4, lw5 = 0.1, 0.1, 100, 100, 100
    num_epochs = 200
        ...
        for batch_idx in np.array_split(train_idx, len(train_idx) // 1):
            ...
            for idx in batch_idx:
                ...
                for neighbor_idx in neighbors[idx]:
                    ...
                    dscp_norm += torch.mean(dscp_gate)
...
if __name__ == "__main__":
    ...
    args.data_path = './data/Yearbook/dataset.pkl'
    args.save_dir = './save/' + 'pred_yearbook_' + datetime.now().strftime("%y%m%d%H%M%S%f")[:-3]
    device = f"cuda:{args.device}" if torch.cuda.is_available() else "cpu"
    os.makedirs(args.save_dir, exist_ok=True)
    currentPython = sys.argv[0]
    shutil.copy2(currentPython, args.save_dir)
    shutil.copy2('utils_yearbook.py', args.save_dir)
