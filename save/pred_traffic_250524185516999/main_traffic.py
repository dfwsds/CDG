...
def main(args, device):
    ...
    if args.dataset in ["moons", "mnist"]:
        ...
        meta_vars = meta_vars / (meta_vars.max(axis=1, keepdims=True) + 1e-8)
    ...
    # Training
    lw1, lw2, lw3, lw4, lw5 = 10, 10, 100, 10, 100
    num_epochs = 600
...
if __name__ == "__main__":
    ...
    args.data_path = './data/Traffic/dataset.pkl'
    args.save_dir = './save/' + 'pred_traffic_' + datetime.now().strftime("%y%m%d%H%M%S%f")[:-3]
    device = f"cuda:{args.device}" if torch.cuda.is_available() else "cpu"
    os.makedirs(args.save_dir, exist_ok=True)
    currentPython = sys.argv[0]
    shutil.copy2(currentPython, args.save_dir)
    shutil.copy2('utils_traffic.py', args.save_dir)
