...
class NeuralLTO(nn.Module):
    def __init__(self, dim, meta_input_dim, n_mani=3, hidden=128):
        super().__init__()
        self.dim = dim
        self.n_mani = n_mani
...
def main(args, device):
    ...
    for epoch in range(num_epochs):
        epoch_loss = 0.0
        shared_model.train()
        for batch_idx in np.array_split(train_idx, len(train_idx) // 2):
            optimizer.zero_grad()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--device", type=int, default=3, help="GPU")
    parser.add_argument("--dataset", type=str, default="mnist",
                        choices=["moons", "mnist", "fmow", "arxiv", "yearbook", "traffic"])

    args = parser.parse_args()
    args.data_path = './data/MNIST/dataset.pkl'
    args.save_dir = './save/' + 'pred_mnist_' + datetime.now().strftime("%y%m%d%H%M%S%f")[:-3]
    device = f"cuda:{args.device}" if torch.cuda.is_available() else "cpu"
    os.makedirs(args.save_dir, exist_ok=True)
    currentPython = sys.argv[0]
    shutil.copy2(currentPython, args.save_dir)
    shutil.copy2('utils_mnist.py', args.save_dir)
